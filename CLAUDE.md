...
edit then Save As New or replace in place), and the editor's Record button (`Audio/AudioRecorder` — taps
`AudioEditorPlayer`'s mix bus and streams the editor's playback to a new 24-bit WAV in a remembered folder).
File writes go through `Audio/AudioFileIO` (temp → validate → atomic replace; a non-WAV source becomes a
sibling `.wav`).
